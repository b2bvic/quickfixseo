#!/usr/bin/env python3
"""
Obsidian SEO Analyzer - Streamlined intelligent SEO audit system
Generates comprehensive markdown SEO audit reports directly to Obsidian vault
No CSV exports, no API dependencies, just intelligent analysis -> markdown
"""
...
class ObsidianSEOAnalyzer:
    def __init__(self, obsidian_vault_path=None):
        ...
        # Obsidian vault setup - check saved config first
        if obsidian_vault_path:
            self.obsidian_vault_path = obsidian_vault_path
        else:
            self.obsidian_vault_path = self.load_or_setup_obsidian_integration()
    ...
    def load_or_setup_obsidian_integration(self):
        """Load saved vault path or setup new one"""
        ...
        if saved_vault and os.path.exists(saved_vault):
            ...
            if saved_vault:
                print(f"⚠️  Saved vault path no longer exists: {saved_vault}")
            return self.setup_obsidian_integration()
    ...
    def setup_obsidian_integration(self):
        ...
        while True:
            try:
                ...
                if choice == "0":
                    desktop_path = os.path.expanduser("~/Desktop/SEO_Audits")
                    # Save the Desktop fallback choice
                    config = self.load_config()
                    config['obsidian_vault_path'] = desktop_path
                ...
                if choice.isdigit() and 1 <= int(choice) <= len(common_paths):
                    ...
                    if os.path.exists(selected_path):
                        ...
                        config = self.load_config()
                        config['obsidian_vault_path'] = selected_path
                        self.save_config(config)
                        print(f"💾 Saved vault location for future use")
                ...
                if int(choice) == len(common_paths) + 1:
                    ...
                    elif not custom_path.startswith('/'):
                    ...
                    if os.path.exists(custom_path):
                        ...
                        # Save the successful path
                        config = self.load_config()
                        config['obsidian_vault_path'] = custom_path
                        self.save_config(config)
                        print(f"💾 Saved vault location for future use")
                        return custom_path
                    else:
                        print(f"❌ Path doesn't exist: {custom_path}")
                        print("💡 Try: ~/Documents/YourVaultName or /full/path/to/vault")
                        continue
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 obsidian_seo_analyzer.py <website_url> [--reconfigure]")
        print("Example: python3 obsidian_seo_analyzer.py example.com")
        print("Options:")
        print("  --reconfigure    Reset and reconfigure Obsidian vault location")
        sys.exit(1)
    ...
    if reconfigure:
        ...
        if os.path.exists(config_file):
            ...
            print("🔄 Cleared saved configuration. Will prompt for new vault location.")
    
    analyzer = ObsidianSEOAnalyzer()
